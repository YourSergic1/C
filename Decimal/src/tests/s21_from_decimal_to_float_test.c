...
START_TEST(from_decimal_to_float_test_1) {
  for (size_t i = 0; i < sizeof(src) / sizeof(s21_decimal); i++) {
    float convert;
    int res = s21_from_decimal_to_float(src[i], &convert);
    ck_assert_float_eq(convert, dst[i]);
...
Suite* s21_from_decimal_to_float_suite(void) {
  Suite* s;
  s = suite_create("\033[33ms21_from_decimal_to_float\033[0m");
