...
START_TEST(is_less_or_equal_test_1) {
  ...
  for (size_t i = 0; i < sizeof(values_1) / sizeof(s21_decimal); i++) {
    ck_assert_int_eq(TRUE, s21_is_less_or_equal(values_1[i], values_2[i]));
  }
}
END_TEST
...
START_TEST(is_less_or_equal_test_2) {
  ...
  for (size_t i = 0; i < sizeof(values_1) / sizeof(s21_decimal); i++) {
    ck_assert_int_eq(FALSE, s21_is_less_or_equal(values_1[i], values_2[i]));
  }
}
END_TEST
...
Suite* s21_is_less_or_equal_suite(void) {
  Suite* s;
  s = suite_create("\033[33ms21_is_less_or_equal\033[0m");

  TCase* tc_core;
  tc_core = tcase_create("s21_is_less_or_equal_core");
  tcase_add_test(tc_core, is_less_or_equal_test_1);
  tcase_add_test(tc_core, is_less_or_equal_test_2);
